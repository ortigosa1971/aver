// server.js — versión final con actualización automática de lluvia
// Adaptado para Railway (con control de sesión única y proxy Weather.com)
...
const log=(...a)=>process.env.DEBUG_SINGLE_SESSION==='1'&&console.log('[session]',...a);
...
app.post('/login',async(req,res)=>{
  try{
    const user=req.body.usuario||req.body.username;
    const pass=req.body.password;
    if(!user||!pass) return res.redirect('/login.html?error=credenciales');
    const found=autenticar(user,pass);
    if(!found) return res.redirect('/login.html?error=credenciales');
    if(found.session_id){
    ...
    log('login ok',user,req.sessionID);
    res.redirect('/inicio');
...
async function requiereSesionUnica(req,res,next){
  try{
    ...
    if(row.session_id!==req.sessionID){
      req.session.destroy(()=>res.redirect('/login.html?error=conectado_en_otra_maquina'));return;
    }
...
app.post('/logout',(req,res)=>{
  ...
  req.session.destroy(()=>{if(u)db.prepare('UPDATE users SET session_id=NULL WHERE username=?').run(u);res.redirect('/login.html?msg=logout');});
});

// ====== API de lluvia WU ======
app.get('/api/lluvia/total/year',requiereSesionUnica,async(req,res)=>{
  try{
    const apiKey=process.env.WU_API_KEY,stationId=process.env.WU_STATION_ID;
    if(!apiKey||!stationId) return res.status(400).json({error:'config_missing'});
    const now=new Date(),y=now.getFullYear();
    const pad=n=>String(n).padStart(2,'0');
    const url=`https://api.weather.com/v2/pws/history/daily?stationId=${stationId}&format=json&units=m&startDate=${y}0101&endDate=${y}${pad(now.getMonth()+1)}${pad(now.getDate())}&apiKey=${apiKey}`;
    const r=await fetch(url);if(!r.ok)throw new Error(r.status);const data=await r.json();
    const arr=Array.isArray(data?.observations)?data.observations:[];
    const total=arr.reduce((a,d)=>a+(+d?.metric?.precipTotal||0),0);
    res.json({total_mm:Number(total.toFixed(2)),year:y});
  }catch(e){console.error(e);res.status(500).json({error:'calc_failed'});}
});

// ====== Proxy condiciones actuales ======
...
async function actualizarLluviaAutomatica(){
  try{
    ...
    const resp=await fetch(baseUrl);const data=await resp.json();
    if(data?.observations?.[0]?.metric?.precipTotal!=null){
      ...
      const rainDB=new Database(check);rainDB.pragma('journal_mode=wal');
  ...
  }catch(err){console.error("⚠️ Error al actualizar lluvia automática:",err);}
}
setInterval(actualizarLluviaAutomatica,6*60*60*1000); // cada 6h
